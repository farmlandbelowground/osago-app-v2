// Vercel Cron — dagelijks. Vindt subscriptions die 7 dagen geleden zijn
// verlopen (en niet zijn verlengd) en stuurt de `subscription_expired_
// followup`-template. Idempotent via `sent_reminders`.
...
export default async function handler(req, res){
  ...
  // Klanten waarvan de sub 7 dagen geleden precies verliep.
  ...
  for(const sub of subs){
    try {
      const key = `expired_followup_${sub.end_date}`;
      ...
      const send = await sendTemplatedMail('subscription_expired_followup', profile.email, {
        ...
        context: 'cron.subscription-expired-followup',
